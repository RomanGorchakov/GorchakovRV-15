...
5. select count(*) from wal_test;
6. select pg_relation_size('wal_test') as size_in_bytes, current_setting('block_size')::int as block_size, pg_relation_size('wal_test') / current_setting('block_size')::int as pages_on_disk;
7. select count(*) AS buffers_in_cache from pg_buffercache b join pg_class c on b.relfilenode = c.relfilenode where c.relname = 'wal_test';
